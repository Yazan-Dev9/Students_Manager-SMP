...
class Management:
    ...
    def get_students_by_class(self, class_id: int):
        students = self._db.execute_query(
    ...
    def get_role_by_name(self, name: str):
        role = self._db.execute_query(
            "SELECT * FROM Role WHERE role_name = ?", (name,)
        )
        return role

    def get_role_by_id(self, id: int):
        role = self._db.execute_query("SELECT * FROM Role WHERE role_id = ?", (id,))
        return role
    ...
    def get_all_teachers(self):
        teachers = self._db.execute_query("SELECT * FROM Teacher")
        return teachers

    def get_all_classes(self):
        classes = self._db.execute_query("SELECT * FROM Classroom")
        return classes

    def get_all_subjects(self):
        subjects = self._db.execute_query("SELECT * FROM Subject")
        return subjects
